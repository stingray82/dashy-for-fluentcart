...
define('RUP_DASHY_FC_VERSION', '1.0.6');
...
/**
 * Determine whether a post type can be used as Dashy content.
 *
 * Attachments/media are intentionally excluded because Dashy renders
 * WordPress post content, not raw attachment files.
 */
function rup_dashy_fc_is_supported_post_type( $post_type ) {
...
function rup_dashy_fc_evenodd_path_to_nonzero( $d ) {
    ...
    while ( $i < count( $tokens ) ) {
        ...
        switch ( $upper ) {
            ...
            case 'C':
                $c1 = array(
                    $relative ? $x + $vals[0] : $vals[0],
                    $relative ? $y + $vals[1] : $vals[1],
                );

                $c2 = array(
                    $relative ? $x + $vals[2] : $vals[2],
                    $relative ? $y + $vals[3] : $vals[3],
                );

                $to = array(
                    $relative ? $x + $vals[4] : $vals[4],
                    $relative ? $y + $vals[5] : $vals[5],
                );
            ...
            case 'S':
                $c1 = $last_cubic_ctrl
                    ? array(
                        2 * $x - $last_cubic_ctrl[0],
                        2 * $y - $last_cubic_ctrl[1],
                    )
                    : array( $x, $y );
            ...
            case 'T':
                $c = $last_quad_ctrl
                    ? array(
                        2 * $x - $last_quad_ctrl[0],
                        2 * $y - $last_quad_ctrl[1],
                    )
                    : array( $x, $y );
            ...
            case 'A':
                $to = array(
                    $relative ? $x + $vals[5] : $vals[5],
                    $relative ? $y + $vals[6] : $vals[6],
                );
    ...
    foreach ( $subpaths as $index => $sub ) {
        ...
        if ( $reverse && ! empty( $segments ) ) {
            ...
            $parts[] = 'M'
                . rup_dashy_fc_svg_number( $start[0] )
                . ' '
                . rup_dashy_fc_svg_number( $start[1] );

            foreach ( array_reverse( $segments ) as $seg ) {
                if ( 'L' === $seg['type'] ) {
                    $parts[] = 'L'
                        . rup_dashy_fc_svg_number( $seg['from'][0] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['from'][1] );
                } elseif ( 'C' === $seg['type'] ) {
                    $parts[] = 'C'
                        . rup_dashy_fc_svg_number( $seg['c2'][0] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['c2'][1] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['c1'][0] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['c1'][1] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['from'][0] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['from'][1] );
                    ...
                    $parts[] = 'Q'
                        . rup_dashy_fc_svg_number( $seg['c'][0] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['c'][1] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['from'][0] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['from'][1] );
                    ...
                    $parts[] = 'A'
                        . rup_dashy_fc_svg_number( $seg['rx'] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['ry'] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['rot'] )
                        . ' '
                        . (int) $seg['large']
                        . ' '
                        . ( $seg['sweep'] ? 0 : 1 )
            ...
            $parts[] = 'M'
                . rup_dashy_fc_svg_number( $sub['start'][0] )
                . ' '
                . rup_dashy_fc_svg_number( $sub['start'][1] );

            foreach ( $segments as $seg ) {
                if ( 'L' === $seg['type'] ) {
                    $parts[] = 'L'
                        . rup_dashy_fc_svg_number( $seg['to'][0] )
                        . ' '
                        . rup_dashy_fc_svg_number( $seg['to'][1] );
                } elseif ( 'C' === $seg['type'] ) {
                    $parts[] = 'C'
                        . rup_dashy_fc_svg_number( $seg['c1'][0] )
                        . ' '
...
function rup_dashy_fc_flatten_evenodd_paths( $svg ) {
    return preg_replace_callback(
        ...
        function( $match ) {
            ...
            $converted = rup_dashy_fc_evenodd_path_to_nonzero(
                html_entity_decode( $d_match[2], ENT_QUOTES )
            );

            $attrs = preg_replace(
                '/\bd\s*=\s*(["\'])(.*?)\1/is',
                'd="' . esc_attr( $converted ) . '"',
                $attrs,
                1
            );

            $attrs = preg_replace(
                '/\s+(?:fill-rule|clip-rule)\s*=\s*(["\']).*?\1/i',
                '',
                $attrs
            );
...
function rup_dashy_fc_transform_path_d( $d, $scale, $tx, $ty ) {
    ...
    while ( $i < count( $tokens ) ) {
        ...
        $delta = function( $value ) use ( $scale ) {
            return $value * $scale;
        };
...
/**
 * Bake the source viewBox into a fixed 20x20 coordinate system.
 */
function rup_dashy_fc_bake_viewbox( $svg ) {
    ...
    $inner = preg_replace_callback(
        ...
        function( $path_match ) use ( $scale, $tx, $ty ) {
            ...
            $new_d = rup_dashy_fc_transform_path_d(
                html_entity_decode( $d_match[2], ENT_QUOTES ),
                $scale,
                $tx,
                $ty
    ...
    $attrs = preg_replace(
        '/\s+(?:width|height|viewBox|viewbox)\s*=\s*(["\']).*?\1/i',
        '',
        $attrs
    );
 ...
 * customer navigation colour scheme.
 */
...
function rup_dashy_fc_normalize_svg( $svg, $normalize_colours = false ) {
    $svg = rup_dashy_fc_flatten_evenodd_paths(
        (string) $svg
    );
    ...
    $is_stroke_icon = (bool) preg_match(
        '/\bstroke\s*=\s*["\'][^"\']+["\']/i',
        $svg
    );
    ...
    if ( preg_match( '/<svg\b([^>]*)>/i', $svg, $match ) ) {
        ...
        if ( ! preg_match( '/\bclass\s*=/i', $attrs ) ) {
            $attrs .= ' class="dashy-fc-nav-icon'
                . ( $is_stroke_icon ? ' dashy-fc-nav-icon--stroke' : '' )
                . '"';
        } else {
...
function rup_dashy_fc_icon_svg( $icon, $normalize_colours = false ) {
    ...
    if ( '' === $icon ) {
        return rup_dashy_fc_normalize_svg(
            rup_dashy_fc_default_icon_svg(),
            $normalize_colours
        );
    }

    if ( str_starts_with( ltrim( $icon ), '<svg' ) ) {
        $svg = rup_dashy_fc_normalize_svg(
            $icon,
            $normalize_colours
        );

        return $svg
            ?: rup_dashy_fc_normalize_svg(
                rup_dashy_fc_default_icon_svg(),
                $normalize_colours
            );
    }
...
/**
 * Keep Dashy icons at the same visual size as FluentCart's native icons.
 */
add_action(
    'fluent_cart/customer_dashboard/enqueue_assets',
    function() {
        $css = '
            .dashy-fc-nav-icon {
            ...
            svg.dashy-fc-nav-icon--stroke {
                fill:none !important;
            }
        ';

        wp_register_style(
            'dashy-for-fluentcart-icons',
            false,
            array(),
            RUP_DASHY_FC_VERSION
        );
...
function rup_dashy_fc_sanitize_tabs( $tabs ) {
    ...
    foreach ( $tabs as $tab ) {
        ...
        if (
            ! $name
            || ! $slug
            || in_array( $slug, $used_slugs, true )
        ) {
            continue;
        }

        $content_type = sanitize_key(
            $tab['content_type'] ?? 'page'
        );
        ...
        if ( ! rup_dashy_fc_is_supported_post_type( $post_type ) ) {
            $post_type = 'page';
        }
        ...
        $insert_after = sanitize_key(
            $tab['insert_after'] ?? 'last'
        );
        ...
        $normalize_icon_colours = ! empty(
            $tab['normalize_icon_colours']
        )
            ? 1
            : 0;
...
add_action(
    ...
    function() {
        register_setting(
            ...
            RUP_DASHY_FC_OPTION,
            array(
                'sanitize_callback' => 'rup_dashy_fc_sanitize_tabs',
            )
        );
    }
);
 ...
 */
add_action(
    'wp_ajax_rup_dashy_fc_get_content_sources',
    function() {
        check_ajax_referer(
            'rup_dashy_fc_content_sources',
            'nonce'
        );
        ...
        if ( ! rup_dashy_fc_is_supported_post_type( $post_type ) ) {
            wp_send_json_error(
                array(
                    'message' => __(
                        'Invalid or unsupported post type.',
                        'dashy-for-fluentcart'
                    ),
                ),
                400
...
function rup_dashy_fc_render_tab_content( $tab ) {
    ...
    if ( 'shortcode' === ( $tab['content_type'] ?? '' ) ) {
        ...
        if ( $post instanceof WP_Post ) {
            ...
            echo '<p>'
                . esc_html__(
                    'Content not found. Check the selected content source in Dashy settings.',
                    'dashy-for-fluentcart'
                )
                . '</p>';
        }
    ...
    function() {
        ...
        foreach ( $tabs as $tab ) {
            ...
            $endpoint = array(
                ...
                'icon_svg' => rup_dashy_fc_icon_svg(
                    $tab['icon'] ?? '',
                    ! empty(
                        $tab['normalize_icon_colours']
                    )
            ...
            $api->addCustomerDashboardEndpoint(
                $slug,
                $endpoint
            );
        }
...
add_filter(
    ...
    function( $items, $context = array() ) {
        ...
        foreach ( $items as $key => $item ) {
            ...
            if ( $clean_key ) {
                $observed[ $clean_key ] = rup_dashy_fc_menu_label(
                    $clean_key,
                    $item
                );
            }
...
add_action(
    ...
    function() {
        ...
        if (
            ...
            || class_exists( '\\FluentCart\\Api\\FluentCartGeneralApi' )
        ) {
            return;
        }
        ...
        if ( 'dashy-for-fluentcart' === $page ) {
            echo '<div class="notice notice-warning"><p>'
                . esc_html__(
                    'Dashy For FluentCart is ready, but FluentCart does not appear to be active. Custom tabs will register when FluentCart is available.',
                    'dashy-for-fluentcart'
                )
                . '</p></div>';
        }
...
function rup_dashy_fc_render_admin_page() {
    ...
    /*
     * Remove unsupported public post types such as Media/Attachments.
     */
    $post_types = array_filter(
        ...
        function( $post_type ) {
            return (
                ...
                && rup_dashy_fc_is_supported_post_type(
                    $post_type->name
                )
            );
        }
    ...
    <div class="wrap">
        ...
        <p>
            <?php
            esc_html_e(
                'Add native FluentCart customer dashboard tabs that render a WordPress page/post/custom post type or a shortcode.',
                'dashy-for-fluentcart'
            );
            ?>
        </p>
        ...
        <p>
            <strong>
                <?php esc_html_e( 'Icons:', 'dashy-for-fluentcart' ); ?>
            </strong>

            <?php
            esc_html_e(
                'FluentCart uses an SVG-only icon slot. Paste inline SVG or select an SVG attachment from the Media Library; raster formats are not accepted. Enable Match Dashboard Colour per tab when you want the SVG to inherit the FluentCart navigation colour.',
                'dashy-for-fluentcart'
            );
            ?>
        </p>
        ...
        <form method="post" action="options.php">
            ...
            <p>
                <button
                    ...
                    class="button"
                >
                    <?php esc_html_e( 'Add Tab', 'dashy-for-fluentcart' ); ?>
                </button>
            </p>
    ...
    <style>
        #rup-dashy-fc-tabs input[type=text],
        #rup-dashy-fc-tabs select {
            width:100%;
            min-width:120px;
        }

        #rup-dashy-fc-tabs td {
            vertical-align:top;
        }

        #rup-dashy-fc-tabs .rup-dashy-fc-icon {
            min-width:220px;
        }

        #rup-dashy-fc-tabs .rup-dashy-fc-source {
            min-width:230px;
        }

        #rup-dashy-fc-tabs .rup-dashy-fc-icon-preview svg {
            width:20px;
            height:20px;
            margin-top:6px;
    ...
    jQuery(function($){
        ...
        var positionOptions = <?php
            $pos_html = '<option value="last">Last item</option>'
                . '<option value="first">First item</option>';

            foreach ( $positions as $key => $label ) {
                $pos_html .= '<option value="'
                    . esc_attr( $key )
                    . '">After '
                    . esc_html( $label )
                    . '</option>';
        ...
        function pageField(i, value) {
            return '<input type="hidden"'
                + ' class="rup-dashy-fc-source-value"'
                + ' name="' + sourceName(i) + '"'
                + ' value="' + esc(value) + '">'
                + '<select'
                + ' class="rup-dashy-fc-source-select rup-dashy-fc-source"'
                + ' data-selected="' + esc(value) + '">'
                + '<option>Loading…</option>'
                + '</select>';
        }

        function shortcodeField(i, value) {
            return '<input type="text"'
                + ' class="rup-dashy-fc-source"'
                + ' name="' + sourceName(i) + '"'
                + ' value="' + esc(value) + '"'
        ...
        function loadSources($row) {
            ...
            $.post(
                ajaxurl,
                {
                    action: 'rup_dashy_fc_get_content_sources',
                    nonce: nonce,
                    post_type: $row.find('.rup-dashy-fc-post-type').val()
                }
            )
            .done(function(resp){
                var html = '<option value="">Select content…</option>';
                var matched = false;
        ...
        $('#rup-dashy-fc-add').on(
            ...
            function(){
                ...
                var row = '<tr>'
                    + '<td>'
                    + '<input type="text"'
                    + ' name="rup_dashy_fc_tabs['+i+'][name]"'
                    + ' required>'
                    + '</td>'

                    + '<td>'
                    + '<input type="text"'
                    + ' name="rup_dashy_fc_tabs['+i+'][slug]"'
                    + ' placeholder="support">'
                    + '</td>'

                    + '<td>'
                    + '<input type="text"'
                    + ' class="rup-dashy-fc-icon"'
                    + ' name="rup_dashy_fc_tabs['+i+'][icon]"'
                    + ' placeholder="Inline SVG or Media Library SVG">'
                    + '<p>'
                    + '<button type="button"'
                    + ' class="button rup-dashy-fc-media">'
                    + 'Select SVG'
                    + '</button>'
                    + '</p>'
                    + '</td>'
                    ...
                    + '<label>'
                    + '<input type="checkbox"'
                    + ' name="rup_dashy_fc_tabs['+i+'][normalize_icon_colours]"'
                    + ' value="1">'
                    + ' Use dashboard colour'
                    + '</label>'
                    + '</td>'

                    + '<td>'
                    + '<select'
                    + ' class="rup-dashy-fc-content-type"'
                    + ' name="rup_dashy_fc_tabs['+i+'][content_type]">'
                    + '<option value="page">'
                    + 'Page/Post/Custom Post'
                    + '</option>'
                    ...
                    + '<td>'
                    + '<select'
                    + ' class="rup-dashy-fc-post-type"'
                    + ' name="rup_dashy_fc_tabs['+i+'][post_type]">'
                    + postTypeOptions
                    + '</select>'
                    + '</td>'
                    ...
                    + '<td>'
                    + '<select'
                    + ' name="rup_dashy_fc_tabs['+i+'][insert_after]">'
                    + positionOptions
                    + '</select>'
                    + '</td>'
                    ...
                    + '<td>'
                    + '<button type="button"'
                    + ' class="button rup-dashy-fc-remove">'
                    + 'Remove'
                    + '</button>'
                    + '</td>'
        ...
        $('#rup-dashy-fc-tabs').on(
            'change',
            '.rup-dashy-fc-source-select',
            function(){
                $(this)
                    .siblings(
                        '.rup-dashy-fc-source-value'
                    )
                    .val(
                        $(this).val()
                    );
                ...
                frame.on(
                    ...
                    function(){
                        ...
                        if (
                            ...
                            window.alert(
                                'FluentCart dashboard icons must be SVG files.'
                            );
        ...
        $('#rup-dashy-fc-tabs tbody tr').each(
            function(){
                var $row = $(this);
...
/**
 * Render an existing admin row.
 */
function rup_dashy_fc_render_admin_row(
    $index,
    $tab,
    $post_types,
    $positions
    ...
    <tr>
        <td>
            <input
                type="text"
                name="<?php echo esc_attr( RUP_DASHY_FC_OPTION ); ?>[<?php echo esc_attr( $index ); ?>][name]"
        ...
        <td>
            <input
                type="text"
                name="<?php echo esc_attr( RUP_DASHY_FC_OPTION ); ?>[<?php echo esc_attr( $index ); ?>][slug]"
        ...
        <td>
            <input
                type="text"
                class="rup-dashy-fc-icon"
                name="<?php echo esc_attr( RUP_DASHY_FC_OPTION ); ?>[<?php echo esc_attr( $index ); ?>][icon]"
            ...
            <p>
                <button
                    ...
                    class="button rup-dashy-fc-media"
                >
                    <?php esc_html_e( 'Select SVG', 'dashy-for-fluentcart' ); ?>
                </button>
            </p>
        </td>
        ...
        <td>
            <label>
                <input
                    type="checkbox"
                    name="<?php echo esc_attr( RUP_DASHY_FC_OPTION ); ?>[<?php echo esc_attr( $index ); ?>][normalize_icon_colours]"
                    value="1"
                    <?php checked( ! empty( $tab['normalize_icon_colours'] ) ); ?>
                >
            ...
            <p class="description">
                <?php
                esc_html_e(
                    'Converts SVG fill/stroke colours to currentColor so the icon follows FluentCart navigation styling.',
                    'dashy-for-fluentcart'
                );
                ?>
            </p>
        ...
        <td>
            <select
                class="rup-dashy-fc-content-type"
                name="<?php echo esc_attr( RUP_DASHY_FC_OPTION ); ?>[<?php echo esc_attr( $index ); ?>][content_type]"
            >
                <option
                    value="page"
        ...
        <td
            style="opacity:<?php echo 'page' === $type ? '1' : '.45'; ?>"
        >
            <select
                class="rup-dashy-fc-post-type"
                name="<?php echo esc_attr( RUP_DASHY_FC_OPTION ); ?>[<?php echo esc_attr( $index ); ?>][post_type]"
                <?php disabled( 'page' !== $type ); ?>
            >
                <?php foreach ( $post_types as $pt ) : ?>
                    <option
                        value="<?php echo esc_attr( $pt->name ); ?>"
                        <?php selected( $tab['post_type'] ?? 'page', $pt->name ); ?>
                    >
                        <?php
                        echo esc_html(
        ...
        <td class="rup-dashy-fc-source-cell">
            <?php if ( 'shortcode' === $type ) : ?>
                <input
                    type="text"
                    class="rup-dashy-fc-source"
                    name="<?php echo esc_attr( RUP_DASHY_FC_OPTION ); ?>[<?php echo esc_attr( $index ); ?>][content_source]"
            ...
            <?php else : ?>
                ...
                <select
                    class="rup-dashy-fc-source-select rup-dashy-fc-source"
                    data-selected="<?php echo esc_attr( $tab['content_source'] ?? '' ); ?>"
                >
                    <option>
                        <?php esc_html_e( 'Loading…', 'dashy-for-fluentcart' ); ?>
                    </option>
                </select>
            <?php endif; ?>
        </td>
        ...
        <td>
            <select
                name="<?php echo esc_attr( RUP_DASHY_FC_OPTION ); ?>[<?php echo esc_attr( $index ); ?>][insert_after]"
            >
                <option
                    value="last"
                    <?php selected( $tab['insert_after'] ?? 'last', 'last' ); ?>
                >
                    <?php esc_html_e( 'Last item', 'dashy-for-fluentcart' ); ?>
                </option>

                <option
                    value="first"
                    <?php selected( $tab['insert_after'] ?? 'last', 'first' ); ?>
                >
                    <?php esc_html_e( 'First item', 'dashy-for-fluentcart' ); ?>
                </option>

                <?php foreach ( $positions as $key => $label ) : ?>
                    <option
                        value="<?php echo esc_attr( $key ); ?>"
                        <?php selected( $tab['insert_after'] ?? 'last', $key ); ?>
                    >
                        <?php
                        echo esc_html(
                            'After ' . $label
        ...
        <td>
            <button
                ...
                class="button rup-dashy-fc-remove"
            >
                <?php esc_html_e( 'Remove', 'dashy-for-fluentcart' ); ?>
            </button>
        </td>
    </tr>
    <?php
